...
namespace main_savitch_14
{
	...
	string game::get_user_move() const
	{
		...
		do
		{
			...
			if (answer.length() != 2 || !isalpha(answer[0]) || !isdigit(answer[1]))
				cout << "ERROR invalid entry: enter only column than row (EX: A1)" << endl;

		} while (answer.length() != 2 || !isalpha(answer[0]) || !isdigit(answer[1]));
	...
	void game::make_human_move()
	{
		...
		while (!is_legal(move))
		{
			display_message("Illegal move.\nif no possible moves available enter (skip)\n");
			move = get_user_move();
		}
